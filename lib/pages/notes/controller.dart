...
class NotesPageController extends GetxController {
  final state = NotesPageState();

  ///添加一个笔记到列表中
  Future<bool> addNote(BaseNote baseNote) async {
    ///持久化笔记到SP中,这里会更新UI
    bool result =
        await DataManager.addNoteCfgPosByNote(state.noteDataList, baseNote);
    if (result == false) {
      return false;
    }
    return true;
  ...
  Future<bool> removeNote(BaseNote baseNote) async {
    ...
    ///删除SP中的记录
    result =
        await DataManager.removeNoteCfgPosByNote(state.noteDataList, baseNote);
    if (result == false) {
      return false;
    }
    return true;
  ...
  Future<bool> updateNote(BaseNote baseNote) async {
    ///更新笔记项目配置文件
    bool result =
        await FileTool.writeMapToFile(baseNote.noteCfgPos, baseNote.toJson());
    if (result == false) {
      return false;
    }
  ...
  Future<void> createNoteProject(
    ...
    BaseNote baseNote = BaseNote(
        ...
        noteCfgPos:
            "${noteProjectPos}/${noteTitle}.${Mconstant.NOTE_CFG_SUFFIX}",
        noteTitle: noteTitle);
    ...
    bool result =
        await FileTool.writeFile(baseNote.noteDataPos, '[{"insert":""}]');
    if (result == false) {
      return;
    }
  ...
  Future<void> openNoteProject(
    ...
    if (!FileTool.fileExists(baseNote.noteDataPos)) {
      ...
      noteContentData = noteContentData.replaceAll(
          oldNoteProjectPos, baseNote.noteProjectPos);
